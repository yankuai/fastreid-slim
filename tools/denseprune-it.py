...
mylog = open('dense-it.log', mode = 'w',encoding='utf-8')

cfg = setup(args)
cfg.defrost()
cfg.MODEL.BACKBONE.PRETRAIN = False
checkpoint_cfg = torch.load('./logs/veri/sbs_Dense40/pruned/pruned_0.3.pth')
...
last_mask = checkpoint_cfg['mask']

print("Original Cfg:", file=mylog)
print(checkpoint_cfg['cfg'], file=mylog)
...
end_mask = last_mask[layer_id_in_cfg]
for layer_id in range(len(model_layer)):
    m = model_layer[layer_id]
    if isinstance(m, nn.BatchNorm2d) and not(isinstance(m, BatchNorm)):
        ...
        if first_bn:
            first_bn = False
            continue
        if isinstance(model_layer[layer_id + 1], channel_selection):
            ...
            if layer_id_in_cfg < len(last_mask):
                end_mask = last_mask[layer_id_in_cfg]
            continue
        else:
...
for k, m in enumerate(model.modules()):
    if isinstance(m, nn.BatchNorm2d):
        ...
        if isinstance(m, BatchNorm):
            continue
        print('m is nn.bn2d',m.weight.data.shape,file=mylog)
        ...
        print('layer index: {:d} \t total channel: {:d} \t remaining channel: {:d}'.
           format(k, mask.shape[0], int(torch.sum(mask))), file=mylog)
...
print("Cfg:", file=mylog)
print(cfg_channel, file=mylog)
...
for layer_id in range(len(old_modules)):
    ...
    if isinstance(m0, nn.BatchNorm2d) and not(isinstance(m0, BatchNorm)):
        ...
        if isinstance(old_modules[layer_id + 1], channel_selection):
            ...
            print('m2',m2.indexes.data.shape)
        ...
        if isinstance(old_modules[layer_id + 1], nn.BatchNorm2d) and isinstance(old_modules[layer_id + 2], nn.ReLU):
            # print('conv+1=bn,conv+2=relu')
            print('w1',w1.shape,type(w1))
            print('idx1',idx1.shape,idx1)
            print('m1',m1.weight.data.shape)
