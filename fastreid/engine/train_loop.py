...
from fastreid.layers import IBN
...
def updateBN(model, sparse_rate):
    for m in model.modules():
        if isinstance(m, nn.BatchNorm2d) and m.weight.requires_grad:
            # print('\n'.join(['%s:%s' % item for item in m.__dict__.items()]))
            # print('***********BNupdate****************\n')
            m.weight.grad.data.add_(sparse_rate*torch.sign(m.weight.data))  # L1
        # elif isinstance(m, IBN) and m.IN.weight.requires_grad and m.BN.weight.requires_grad:
        #     m.BN.weight.grad.data.add_(sparse_rate*torch.sign(m.BN.weight.data))
...
class SimpleTrainer(TrainerBase):
    ...
    def __init__(self, model, data_loader, optimizer, sparsity):
        """
        Args:
            ...
            sparsity: a dict {"sparsity_regularization": boolean, "sparse_rate": double}
        """
        super().__init__()
    ...
    def run_step(self):
        ...
        # 传入args
        if self.sparsity["sparsity_regularization"]:
            updateBN(self.model, self.sparsity["sparse_rate"])
