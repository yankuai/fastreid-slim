...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, bn_norm, cfg_channel, with_ibn=False, with_se=False,
        ...
        if with_ibn:
            ...
            self.bn1 = nn.BatchNorm2d(cfg_channel[0]) #get_norm(bn_norm, cfg_channel[0])
        self.conv2 = nn.Conv2d(cfg_channel[0], cfg_channel[1], kernel_size=3, stride=stride,
                               padding=1, bias=False)
        self.bn2 = nn.BatchNorm2d(cfg_channel[1]) #get_norm(bn_norm, cfg_channel[1])
        self.conv3 = nn.Conv2d(cfg_channel[1], planes * self.expansion, kernel_size=1, bias=False)
        self.bn3 = nn.BatchNorm2d(planes * self.expansion) #get_norm(bn_norm, planes * self.expansion)
        self.relu = nn.ReLU(inplace=True)
        if with_se:
...
class ResNet(nn.Module):
    def __init__(self, last_stride, bn_norm, with_ibn, with_se, with_nl, block, layers, non_layers, cfg_channel=None):
        ...
        self.conv0 = nn.Conv2d(3, 64, kernel_size=7, stride=2, padding=3,
                               bias=False)
        self.bn0 = nn.BatchNorm2d(64) #get_norm(bn_norm, 64)
        self.relu = nn.ReLU(inplace=True)
    ...
    def _make_layer(self, block, planes, blocks, cfg_channel, stride=1, bn_norm="BN", with_ibn=False, with_se=False):
        ...
        if stride != 1 or self.inplanes != planes * block.expansion:
            downsample = nn.Sequential(
                ...
                nn.BatchNorm2d(planes * block.expansion) #get_norm(bn_norm, planes * block.expansion),
            )

        layers = []
        layers.append(block(self.inplanes, planes, bn_norm, cfg_channel[0:3], with_ibn, with_se, stride, downsample))
        self.inplanes = planes * block.expansion
        for i in range(1, blocks):
            layers.append(block(self.inplanes, planes, bn_norm, cfg_channel[3*i:3*(i+1)], with_ibn, with_se))
...
def build_resnetbn_backbone(cfg):
    """
